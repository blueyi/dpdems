...
compile_search = os.system('g++ -c -g ./testinputDatas.cpp -std=c++11')

if compile_inputdata == 0 and compile_common == 0 and compile_search == 0 and compile_grid == 0 and compile_config == 0 :
  link = os.system('g++ inputDatas.o common.o testinputDatas.o grid.o config.o -o testinputDatas -std=c++11 -g')
  if link == 0 :
    print('link success!')
    isrun = input('Run? y or n : ')
    if isrun == 'y':
      if platform.system() == 'Windows' : 
        os.system('testinputDatas.exe')
      else :
        os.system('./testinputDatas.exe')
else :
  print('compile error')
